...
from account.auth import Authentication
...
class SignUpView(APIView, Authentication):
    ...
    def post(self, request):
        ...
        if not name or not email or not password:
            raise AuthenticationFailed("Campos são obrigatórios")
                
        signup = self.signup(name, email, password)

        if not signup:
            raise AuthenticationFailed("Ocorreu um erro tente novamente mais tarde")
